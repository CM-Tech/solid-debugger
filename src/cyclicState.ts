import { createSignal, batch, getListener } from "solid-js";
...
type AddSymbolToPrimitive<T> = T extends { [Symbol.toPrimitive]: infer V } ? { [Symbol.toPrimitive]: V } : {};
type AddSymbolIterator<T> = T extends { [Symbol.iterator]: infer V } ? { [Symbol.iterator]: V } : {};
type AddSymbolToStringTag<T> = T extends { [Symbol.toStringTag]: infer V } ? { [Symbol.toStringTag]: V } : {};
...
export function unwrap<T extends StateNode>(item: any, done: Map<any, any> = new Map()): T {
  let result, unwrapped, v, prop;

  if (done.has(item)) return done.get(item);
  if ((result = item != null && item[$RAW])) return result;
  if (!isWrappable(item)) return item;
  if (Array.isArray(item)) {
    let oldItem = item;
    if (Object.isFrozen(item)) item = item.slice(0);
    done.set(oldItem, item);

    for (let i = 0, l = item.length; i < l; i++) {
      v = item[i];
      if ((unwrapped = unwrap(v, done)) !== v) item[i] = unwrapped;
    }
  } else {
    let oldItem = item;
    ...
    for (let i = 0, l = keys.length; i < l; i++) {
      ...
      v = item[prop];
      if ((unwrapped = unwrap(v, done)) !== v) item[prop] = unwrapped;
    }
  }
  return item;
}
...
const proxyTraps: ProxyHandler<StateNode> = {
  get(target, property, receiver) {
    ...
    return wrappable ? wrap(value, "_SOLID_DEV_" && target[$NAME] && `${target[$NAME]}:${property as string}`) : value;
  },
  ...
  if (path.length > 1) {
    ...
    if (Array.isArray(part)) {
      ...
      for (let i = 0; i < current.length; i++) {
        if (part(current[i], i)) updatePath(current, [i].concat(path), ([i] as (number | string)[]).concat(traversed));
      }
      return;
...
type StateSetter<T> =
  | Partial<T>
  | ((prevState: T extends NotWrappable ? T : State<T>, traversed?: (string | number)[]) => Partial<T> | void);
...
export interface SetStateFunction<T> {
  ...
  <K1 extends Part<T>, K2 extends Part<Next<T, K1>>, Setter extends StateSetter<Next<Next<T, K1>, K2>>>(
    ...args: [K1, K2, Setter]
  ): void;
  <
    K1 extends Part<T>,
...
export function createCyclicState<T extends StateNode>(
  state: T | State<T>,
  options?: { name?: string }
