import { Component, createEffect, onMount, JSX, Show, createSignal, createMemo, onCleanup } from "solid-js";
import { Uri, languages, editor as mEditor } from "monaco-editor";
import type { IStandaloneThemeData } from "monaco-editor/esm/vs/editor/editor.api.d.ts";
...
const makeMon = (x: any): IStandaloneThemeData => {
  return {
    base: "vs-dark",
    inherit: false,
    rules: x.tokenColors.map((x) => ({ token: x.scope, ...x.settings })),
    colors: x.colors,
  };
};
const Editor: Component<Props> = (props) => {
  ...
  let model = createMemo(() => mEditor.createModel(props.value || "", "typescript"));
  ...
  return (
    <div
      ...
      style={`grid-template-rows: minmax(0, 1fr) auto;${props.style || ""}`}
    >
      <div style="height:100%;" ref={parent}></div>

      <div class="flex justify-end space-x-2 p-2" classList={{ hidden: !props.canFormat && !props.canCopy }}>
